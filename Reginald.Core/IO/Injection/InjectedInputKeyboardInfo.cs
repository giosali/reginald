namespace Reginald.Core.IO.Injection
{
    ...
    public enum VK : short
    {
        ...
        /// <summary>
        /// Windows 2000/XP: Used to pass Unicode characters as if they were keystrokes. The VK_PACKET key is the low word of a 32-bit Virtual Key value used for non-keyboard input methods. For more information, see Remarks in <see href="https://msdn.microsoft.com/ru-ru/library/windows/desktop/ms646271(v=vs.85).aspx">KEYBDINPUT</see>, <see href="https://msdn.microsoft.com/ru-ru/library/windows/desktop/ms646310(v=vs.85).aspx">SendInput</see>, <see href="https://msdn.microsoft.com/ru-ru/library/windows/desktop/ms646280(v=vs.85).aspx">WM_KEYDOWN</see> and <see href="https://msdn.microsoft.com/ru-ru/library/windows/desktop/ms646281(v=vs.85).aspx">WM_KEYUP</see>.
        /// </summary>
        PACKET = 0xE7,
    ...
    public sealed class InjectedInputKeyboardInfo
    {
        ...
        public static InjectedInputKeyboardInfo FromString(string str)
        {
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                switch (ch)
                {
                    case '\0':
                        break;
                    case '\t':
                        inputs.AddRange(CreateInputs(new VK[] { VK.TAB }));
                        break;
                    case '\n':
                        inputs.AddRange(CreateInputs(new VK[] { VK.SHIFT, VK.RETURN }));
                        break;
                    default:
                        INPUT input = new();
                        input.type = (uint)InputType.Keyboard;
            ...
            InjectedInputKeyboardInfo info = new();
            info.Inputs = inputs.ToArray();
            return info;
        }

        private static INPUT[] CreateInputs(VK[] vks)
        {
            int length = vks.Length;
            INPUT[] inputs = new INPUT[length * 2];
            for (int i = 0; i < length; i++)
            {
                short vk = (short)vks[i];
                INPUT input = new();
                input.type = (uint)InputType.Keyboard;
                input.U = new InputUnion
                {
